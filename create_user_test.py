...
def negative_assert_symbol(first_name):
    ...
    assert responce.json()["code"] == 400
    assert responce.json()["message"] == "Имя пользователя введено некорректно. Имя может содержать только русские или латинские буквы, длина должна быть не менее 2 и не более 15 символов"
...
def negative_assert_no_first_name(user_body):
    ...
    assert responce.status_code == 400
    assert responce.json()["code"] == 400
    assert responce.json()["message"] == "Не все необходимые параметры были переданы"
